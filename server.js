...
function decompress(buf, encoding) {
  ...
  try {
    ...
    console.warn("[decompress] failed:", enc, e.message);
  }
  return buf;
}
...
function rewriteJs(js, base) {
  ...
  js = js.replace(/(["'])(https?:\/\/[^"']{4,}?)\1/g, (match, q, url) => {
...
function buildRuntime(base, targetUrl) {
  ...
  const encodeFn = `
function __uvEncode(s){
  try{
    return btoa(unescape(encodeURIComponent(s)))
      .replace(/\\+/g,'-').replace(/\\//g,'_').replace(/=/g,'');
  }catch(e){return null;}
}
function __uvDecode(s){
  try{
    var p=(4-(s.length%4))%4; s=s+'===='.slice(0,p);
...
function __uvProxify(url){
  ...
  if(/^https?:\\/\\//.test(url)){
    return ${prefixJson}+__uvEncode(url);
  }
  try{
    var abs=new URL(url,${baseJson}).href;
    return ${prefixJson}+__uvEncode(abs);
  }catch(e){return url;}
}
`;
...
(function(){
  ...
  document.createElement = function(tag){
    ...
    if(tag.toLowerCase()==='a'){
      var _setHref = Object.getOwnPropertyDescriptor(HTMLAnchorElement.prototype,'href');
      if(_setHref&&_setHref.set){
        Object.defineProperty(el,'href',{
          set:function(v){ _setHref.set.call(this,__uvProxify(v)); },
          get:function(){ return _setHref.get.call(this); },
          configurable:true
        });
      }
    }
...
async function handleProxyRequest(req, res, targetUrl, redirectCount = 0) {
  ...
  return new Promise((resolve) => {
    ...
    proxyReq.on("error", (err) => {
      console.error("[proxy] request error:", err.message, "→", targetUrl);
...
app.all(UV_CONFIG.prefix + "*", async (req, res) => {
  const raw = req.url.slice(UV_CONFIG.prefix.length - 0);
